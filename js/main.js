...
// ── Ground-edge camera safety ──
// A fixed zoom/tilt limit can't fully prevent seeing past the edge of the
// (finite) ground plane without forcing the camera to an almost-straight-down
// angle everywhere — the math: at a shallow tilt, the top-of-screen ray is
// nearly parallel to the ground and travels enormous distances before it
// would hit y=0. So instead of a static limit, this checks — after every
// pan/zoom/rotate — where the FOUR CORNER rays of the actual current view
// hit the ground, and only pulls the camera in (reduces the orbit radius,
// i.e. a gentle auto zoom-in) just enough to keep all four on the plane.
// Near the center of campus this never engages, so normal use is unchanged;
// it only tightens automatically as the view nears the true map edge.
function groundBounds() {
  const g = campusData.ground;
  return {
    minX: g.offsetX - g.width / 2,
    maxX: g.offsetX + g.width / 2,
    minZ: g.offsetZ - g.depth / 2,
    maxZ: g.offsetZ + g.depth / 2,
  };
}

// True if every corner of the view frustum — at orbit radius r, elevation
// theta, azimuth phi around the current lookTarget — hits the ground plane
// within its bounds (a ray that points at/above horizontal never hits it).
// Plain scalar math (no THREE.Vector3 allocations) since this runs inside a
// ~18-iteration binary search on every pointermove while dragging.
function isRadiusSafe(r, theta, phi) {
  const cosT = Math.cos(theta), sinT = Math.sin(theta);
  const cosP = Math.cos(phi),   sinP = Math.sin(phi);
  const camX = lookTarget.x + r * cosT * cosP;
  const camY = lookTarget.y + r * sinT;
  const camZ = lookTarget.z + r * cosT * sinP;

  // forward = normalize(lookTarget - camPos); since camPos is lookTarget plus
  // r*(cosT*cosP, sinT, cosT*sinP), that direction is just the negation,
  // already unit length.
  ...
  // up = cross(right, forward) — already unit length (right and forward are
  // both unit and perpendicular by construction)
  const ux = -rz * fy, uy = rz * fx - rx * fz, uz = rx * fy;

  const halfV = THREE.MathUtils.degToRad(camera.fov) / 2;    // vertical half-FOV
  const halfH = Math.atan(camera.aspect * Math.tan(halfV));  // horizontal half-FOV
  const tanH = Math.tan(halfH), tanV = Math.tan(halfV);
  const bounds = groundBounds();
  const margin = 2; // keep the true edge just out of frame, not flush with it

  for (const sx of [-1, 1]) {
    for (const sy of [-1, 1]) {
      const dx = fx + rx * tanH * sx + ux * tanV * sy;
      const dy = fy + ry * tanH * sx + uy * tanV * sy;
      const dz = fz + rz * tanH * sx + uz * tanV * sy;
      if (dy >= -0.0001) return false; // points at/above horizontal — never hits ground
      const t = -camY / dy;
      const hitX = camX + t * dx;
      const hitZ = camZ + t * dz;
      if (hitX < bounds.minX + margin || hitX > bounds.maxX - margin ||
          hitZ < bounds.minZ + margin || hitZ > bounds.maxZ - margin) {
        return false;
      }
    }
  }
...
// Largest orbit radius at the given theta/phi (from the current lookTarget)
// that keeps the whole view on the ground. Binary search — the view's ground
// footprint grows monotonically with r for a fixed theta/phi/lookTarget.
// 18 iterations on a [5, 3000] range converges to well under a tenth of a
// unit, more than precise enough at this map's scale.
function maxSafeRadius(theta, phi) {
  let lo = 5, hi = 3000;
  if (!isRadiusSafe(lo, theta, phi)) return lo; // shouldn't happen in practice
  for (let i = 0; i < 18; i++) {
    const mid = (lo + hi) / 2;
    if (isRadiusSafe(mid, theta, phi)) lo = mid; else hi = mid;
  }
  return lo;
}

// Shrinks camOffset (preserving its direction — tilt and azimuth — exactly)
// down to the safe radius for that direction, if it currently exceeds it.
function clampCameraToGround() {
  const r = Math.sqrt(camOffset.x ** 2 + camOffset.y ** 2 + camOffset.z ** 2);
  if (r < 1) return;
  const theta = Math.asin(camOffset.y / r);
  const phi = Math.atan2(camOffset.z, camOffset.x);
  const safeR = maxSafeRadius(theta, phi);
  if (r > safeR) {
    const scale = safeR / r;
    camOffset.x *= scale;
    camOffset.y *= scale;
